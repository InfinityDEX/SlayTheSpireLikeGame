...
/// 
/// アイコンの画像をIDで紐付け管理し、
/// IDを渡すことでアイコンバッチを生成する
/// </summary>
public class IconBatchManager : MonoBehaviour
{
    [field:SerializeField, Header("行動アイコンバッチを配置するオブジェクト")]
    private GameObject actionBatchGroup;

    [field:SerializeField, Header("効果アイコンバッチを配置するオブジェクト")]
    private GameObject effectBatchGroup;

    [field:SerializeField, Header("アイコンバッチのプレハブ")]
    ...
    [field:SerializeField, Header("アイコンリスト")]
    ...
    /// 行動バッチを全て削除
    /// </summary>
    public void ResetActionBatches()
    {
        ResetBatches(actionBatches);
    }

    /// <summary>
    /// 効果バッチを全て削除
    /// </summary>
    public void ResetEffectBatches()
    {
        ResetBatches(effectBatches);
    }

    /// <summary>
    /// 渡したリストにある全てのバッチを削除
    /// </summary>
    /// <param name="batches">アイコンバッチ</param>
    private void ResetBatches(List<IconBatch> batches)
    {
        batches.ForEach(e => {
            if(e != null)
